...
hero:
  ...
  actions:
    ...
    - theme: alt
      ...
      link: /ru/samples/docker-compose
  image:
    src: /logo.png
    alt: Логотип ChokaQ
...
features:
  - icon: storage
    ...
    link: /ru/1-architecture/three-pillars
  - icon: code
...
Если вы впервые смотрите на ChokaQ, начните с [Runtime Model](/ru/runtime-model).
Там объясняется, где ChokaQ работает, как он хранит work и как вызывает ваши
handlers. Затем переходите к [Обзору](/ru/overview) для более широкой картины.

Если хотите сразу запустить код, начните с [Getting Started](/ru/getting-started)
или [Docker Compose Sample](/ru/samples/docker-compose).
