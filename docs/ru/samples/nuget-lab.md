...
Если это app не restore'ится или не runs from local packages, package shape не готова к publish.
...
Из repository root соберите local packages в `artifacts/packages`.
...
Сначала соберите local packages. `NuGet.config` lab указывает на `artifacts/packages`. Если эта folder не содержит `ChokaQ.0.1.0-preview.1` и related internal packages, restore fail'ится.
